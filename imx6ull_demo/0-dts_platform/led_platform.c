...
#define CDEV_CNT 1
#define DTS_NODE "/gpioled0" // root node `gpioled: gpioled0{}` in `.dts` file
#define DTS_COMPATIBLE "gpio-led" // `{compatible = "gpio-led"}`
#define DTS_ND_GPIO                                                            \
  "led-gpio" // `led-gpio = <&gpio1 3 0>` in dts node               for imx6ull
