...
static int __init my_init(void)
{
...
#ifdef STATIC_DEVNR
        dev_nr = STATIC_DEVNR;
        status = register_chrdev_region(dev_nr, MINORMASK + 1, "hello_cdev_test");
#else
        status = alloc_chrdev_region(&dev_nr, 0, MINORMASK + 1, "hello_cdev_test");
#endif
        if (status) {
        ...
        if (!device_create(my_class, NULL, dev_nr, NULL, "hello_test%d", 0)) {
